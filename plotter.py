...
class Plotter2D:
    def __init__(self, grid, colormap='RdPu'):
        ...
        # streamplot start-points
        self.start_points = np.array([[[grid.x.arr[i], grid.y.arr[j]] for i in range(grid.x.arr.shape[0])]
                                      for j in range(grid.y.arr.shape[0])])
        self.start_points = self.start_points.reshape((self.start_points.shape[0] * self.start_points.shape[1], 2))
    ...
    def trigonometric_interpolation_curl(self, vector1, vector2):
        vector1_spectrum, vector2_spectrum = forward_scalar_transform(vector1), forward_scalar_transform(vector2)
        curl_spectrum = 1j * (self.grid.x.device_wavenumbers[:, None] * vector2_spectrum -
                              self.grid.y.device_wavenumbers[None, :] * vector1_spectrum)
        pad_spectrum = cp.zeros((self.grid_x_modes + 2 * self.x_pad_width,
                                 self.grid_y_modes + self.y_pad_width)) + 0j
        pad_spectrum[self.x_pad_width:-self.x_pad_width, :-self.y_pad_width] = curl_spectrum
        return inverse_scalar_transform(pad_spectrum)
    ...
    def stream_plot_interpolated(self, vector_x, vector_y, title, cb_lim=None, save=None):
        if vector_x.arr_nodal is None:
            vector_x.inverse_fourier_transform()
            vector_y.inverse_fourier_transform()

        plot_arr_x = self.trigonometric_interpolation(scalar=vector_x.arr_nodal).get()
        plot_arr_y = self.trigonometric_interpolation(scalar=vector_y.arr_nodal).get()
        magnitude = np.absolute(plot_arr_x**2 + plot_arr_y**2)
        curl = self.trigonometric_interpolation_curl(vector1=vector_x.arr_nodal, vector2=vector_y.arr_nodal).get()
        s_mag = curl  # np.sign(curl) * magnitude  # ``signed magnitude"
        if cb_lim is None:
            cb = cp.linspace(cp.amin(s_mag), cp.amax(s_mag), num=100).get()
        else:
            cb = cp.linspace(cb_lim[0], cb_lim[1], num=100).get()
        plt.figure(figsize=(4, 10))
        plt.contourf(self.XF, self.YF, s_mag, cb, cmap=self.colormap)
        plt.streamplot(self.x_fine, self.y_fine, plot_arr_x.T, plot_arr_y.T, color=magnitude.T, arrowstyle='-|>',
                       start_points=self.start_points, integration_direction='both', density=4)
        plt.xlabel('x'), plt.ylabel('y'), plt.title(title)
        plt.xlim([self.grid.x.arr[0], self.grid.x.arr[-1]]), plt.ylim([self.grid.y.arr[0], self.grid.y.arr[-1]])
        plt.tight_layout()
        # plt.colorbar(), plt.tight_layout()
